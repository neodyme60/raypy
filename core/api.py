...
def create_sampler_random(paramset: ParamSet, film: Film, camera: Camera) -> RandomSampler:
    ...
    sampler = RandomSampler(BucketExtend(0, 0, film.width - 1, film.height - 1), samples, camera.shutterOpen, camera.shutterClose)
    return sampler
...
def create_sampler_stratified(paramset: ParamSet, film: Film, camera: Camera) -> StratifiedSampler:
    ...
    sampler = StratifiedSampler(BucketExtend(0, 0, film.width - 1, film.height - 1), xsamples, ysamples, jitter,camera.shutterOpen, camera.shutterClose)
    return sampler

def create_sampler_lowdiscrepancy(paramset: ParamSet, film: Film, camera: Camera) -> LowDiscrepancySampler:
    sampler = LowDiscrepancySampler(BucketExtend(0, 0, film.width - 1, film.height - 1), 1, camera.shutterOpen, camera.shutterClose)
    return sampler

def create_sampler_halton(paramset: ParamSet, film: Film, camera: Camera) -> HaltonSampler:
    sampler = HaltonSampler(BucketExtend(0, 0, film.width - 1, film.height - 1), 1, camera.shutterOpen, camera.shutterClose)
    return sampler

def create_sampler_bestcandidate(paramset: ParamSet, film: Film, camera: Camera) -> BestcandidateSampler:
    sampler = BestcandidateSampler(BucketExtend(0, 0, film.width - 1, film.height - 1), 1, camera.shutterOpen, camera.shutterClose)
    return sampler

def create_sampler_adaptive(paramset: ParamSet, film: Film, camera: Camera) -> AdaptiveSampler:
    sampler = AdaptiveSampler(BucketExtend(0, 0, film.width - 1, film.height - 1), 1, camera.shutterOpen, camera.shutterClose)
    return sampler
...
def pbrtShape(name:string, params:ParamSet):
    ...
    geom_prim = GeometricPrimitive(shape, mat)
